...
for(let i=0; i<recipes.length; i+=3){
    ...
    groupDiv.classList.add(`groupRecipes${i}`, 'col', '12', 'recipes-cards-part')
    for(let j=i; j<i+3 && j<recipes.length; j++){
        ...
        linkToRecipe.addEventListener('click', ()=>toRecipePage(j));
        linkToRecipe.text="Let's cook"
        linkToRecipe.classList.add('btn', 'btn-primary')
